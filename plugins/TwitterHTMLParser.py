...
SELECT EXISTS(SELECT 1 FROM {TwitterHTMLParser.TableName} WHERE img_url = ?)
""", (url,))
...
class Data:
    ...
    def execute_image(self):
        ...
        try:
            ...
            path = f"{WorkDir}/twitter/{self.author}/{img_hash}.{ext}"
...
class TwitterHTMLParser:
    ...
    def parse(self, html: str):
        ...
        if not re.match(self.url_pattern, url):
            url = soup.find("link", attrs={"rel": "canonical"}).get("href")
    ...
    def execute(self):
        self.shower.update_interface(DataTree().dict())
        for img_url in self.img_urls:
            if db_obj.twitter_exists(img_url):
                continue
            self.data.img_url = img_url
            self.data.execute_image()
            db_obj.insert_data("data", self.data.insert_type())
